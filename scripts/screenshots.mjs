...
await expect(page.getByRole('button', { name: 'Sort my words' })).toBeEnabled();
await shot('capture');
await audit('voice transcript');
await page.getByRole('button', { name: 'Sort my words' }).click();
...
await page.getByRole('button', { name: 'Add 6 to calendar' }).waitFor();
await shot('review');
await audit('PDF review');
await page.getByRole('button', { name: 'Add 6 to calendar' }).click();
...
await page.getByText('0 things found', { exact: true }).waitFor();
await shot('empty');
