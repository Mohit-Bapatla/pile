...
test('selective syllabus review edits in place, exports valid ICS, and rejects duplicates', async ({
  ...
  await expect(dialog.getByLabel('Select Office hours', { exact: true })).not.toBeChecked();
  await expect(dialog.getByLabel('Select UGS 303 class', { exact: true })).toBeChecked();
  ...
  await dialog.getByRole('button', { name: 'Edit candidate Office hours' }).click();
...
test('synced demo event can be edited without creating another event', async ({ page }) => {
  ...
  await page.reload();
  await page.getByRole('button', { name: 'Edit Dentist', exact: true }).click();
